...
export class ClientService {
  ...
  getClientInfo() {
    return this.http.get(`${this.CLIENT_URL}/getInfo`, {
  ...
  sendVerificationCode(code: any) {
    ...
    if (this.attempts_number > 3) {
      this.toast.error('You have acheived the maximum number of attemps', '', {
        timeOut: 2000,
      });
      return;
    } else if (new RegExp('^[0-9]+').test(code) == false) {
      this.toast.error('Invalid verification code', '', { timeOut: 2000 });
      return;
    }
    ...
    this.http
      ...
      .subscribe(
        (response) => {
          ...
          this.toast
            .success('The transaction has been successfully', '', {
              timeOut: 2000,
            })
            .onHidden.subscribe(() => {
  ...
  checkIfTheClientIsConnectedForTheFirstTime(clientToken: any) {
    return this.http.get(`${this.CLIENT_URL}/getStatusOfTheClient`, {
      headers: {
  ...
  setNewPasswordForTheClient(data: any) {
    ...
    return this.http.post(`${this.CLIENT_URL}/clientNewPassword`, data, {
      headers: {
        Authorization: `Bearer ${this.token}`,
      },
    });
  ...
  getMyInfo() {
    return this.http.get(`${this.CLIENT_URL}/clientInfo`, {
      headers: {
        Authorization: `Bearer ${this.token}`,
      },
    });
